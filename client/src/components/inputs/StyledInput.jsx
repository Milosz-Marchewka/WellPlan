const StyledInput = ({id, type="text", label, width = "w-full", mt = "mt-0"})=>{
    return(
        <div className={`relative ${width} ${mt}`}>
            <input type={type} id={id} class={`bg-zinc-500 peer outline-none border border-white rounded-xl px-3 pt-5 pb-2 w-full focus:outline-none focus:border-2 focus:border-emerald-400 ${type=="number" && `[&::-webkit-inner-spin-button]:appearance-none
