...
const StyledSelect = ({id, label, name, value, width = "w-full", mt = "mt-0", options = [], disabled = false, valid = true, onChange})=>{
    ...
    return(
        <div className={`relative ${width} ${mt}`}>
            <select id={id} class={`bg-gray-600 peer text-white outline-none border border-white rounded-xl px-3 pt-5 pb-2 w-full focus:outline-none focus:border focus:border-emerald-400`} name={name} value={value} placeholder=" " disabled={disabled}  onChange={onChange}>
                {
